    ...
    ciudad.listar(req.params.id, req.params.nombre, (err, datos) => {
        ...
        if (err) {
            return res.status(500).send({
                mensaje: 'Error obteniendo la lista de ciudades' });
            }
            else {
...
exports.agregar = (req, res) => {
    ...
    if (!req.body || !req.body.nombre) {
        return res.status(400).send({
            message: 'El contenido del mensaje debe tener información con la Ciudad' });
}
ciudad.agregar(req.params.id, req.params.nombreregion, req.body,
                (err, data) => {
                    ...
                    if (err) {
                        return res.status(500).send({
                            mensaje: 'Error agregando la Ciudad' });
                    }
                    else {
    ...
    exports.modificar = (req, res) => {
        ...
        ciudad.modificar(req.params.id, req.params.nombreregion, req.body,
                    (err, data) => {
                        ...
                        if (err) {
                            return res.status(500).send({
                                mensaje: 'Error actualizando la Ciudad ' });
        }else {
                                //Se devuelve el registro actualizado
                                return res.send(data);
                            }
        ...
        exports.eliminar = (req, res) => {
            ciudad.eliminar(req.params.id, req.params.nombreregion,
                req.params.nombreciudad,
                (err, data) => {
                    //Verificar si hubo error
                    if (err) {
                        return res.status(500).send({
                            mensaje: 'Error eliminando la Ciudad ' });
            }
            else {
                                //Se devuelve mensaje
                                return res.send({
                                    mensaje: `La Ciudad con nombre:${req.params.nombreciudad} fue eliminada`
                                });
                            }
            });
        }