...
exports.listar = (req, res) => {
    region.listar(req.params.id, (err, datos) => {
        ...
        if (err) {
            return res.status(500).send({ mensaje: 'Error obteniendo la lista de regiones' + err });
        }
        else {
...
exports.agregar = (req, res) => {
    ...
    if (!req.body || !req.body.nombre) {
        return res.status(400).send({ message: 'El contenido del mensaje debe tener información con la región' });
    }
...
exports.modificar = (req, res) => {
    ...
    region.modificar(req.params.id, req.body,
        (err, data) => {
            //Verificar si hubo error
            if (err) {
                return res.status(500).send({ mensaje: 'Error actualizando la región ' });
            }
            else {
                //Se devuelve el registro actualizado
                return res.send(data);
...
exports.eliminar = (req, res) => {
    region.eliminar(req.params.id, req.params.nombre,
        (err, data) => {
            //Verificar si hubo error
            if (err) {
                return res.status(500).send({ mensaje: 'Error eliminando la región ' });
            }
            else {
                //Se devuelve mensaje
                return res.send({ mensaje: `La región con nombre:${req.params.nombre} fue eliminada` });
            }
        });
}
